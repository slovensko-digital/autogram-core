...
public class SigningParameters {
    ...
    private SigningParameters(
            ...
            EFormAttributes eFormAttributes, boolean isPlainXml, boolean checkPDFACompliance, int preferredPreviewWidth) {
    ...
    public static SigningParameters buildParameters(
            ...
            int preferredPreviewWidth, DSSDocument document) throws AutogramException {
        ...
        return new SigningParameters(
                ...
                keyInfoCanonicalization, eFormAttributes, isPlainXml, checkPDFACompliance, preferredPreviewWidth);
    }

    public static SigningParameters buildForPDF(DSSDocument document, boolean checkPDFACompliance, boolean signAsEn319132, BaselineLevel baselineLevel) throws AutogramException {
        return buildParameters(
                baselineLevel.getSignatureLevel(SignatureForm.PAdES), DigestAlgorithm.SHA256,
                null, null, signAsEn319132, null, null, null, null, false,
                null, checkPDFACompliance, 640, document);
    }

    public static SigningParameters buildForASiCWithXAdES(DSSDocument document, boolean checkPDFACompliance, boolean signAsEn319132, BaselineLevel baselineLevel, boolean plainXmlEnabled) throws AutogramException {
        var signingParameters = buildParameters(
                baselineLevel.getSignatureLevel(SignatureForm.XAdES), DigestAlgorithm.SHA256,
                ASiCContainerType.ASiC_E, SignaturePackaging.ENVELOPING, signAsEn319132, null, null, null,  null, true,
                EFormUtils.getFsFormIdFromFilename(document.getName()), checkPDFACompliance, 640, document);

        if (!plainXmlEnabled && signingParameters.isPlainXml())
            throw new UnknownEformException();

        return signingParameters;
    }

    public static SigningParameters buildForASiCWithCAdES(DSSDocument document, boolean checkPDFACompliance, boolean signAsEn319132, BaselineLevel baselineLevel, boolean plainXmlEnabled) throws AutogramException {
        var signingParameters = buildParameters(
                baselineLevel.getSignatureLevel(SignatureForm.CAdES), DigestAlgorithm.SHA256,
                ASiCContainerType.ASiC_E, SignaturePackaging.ENVELOPING, signAsEn319132, null, null, null, null, true,
                EFormUtils.getFsFormIdFromFilename(document.getName()), checkPDFACompliance, 640, document);
    ...
    public AbstractSignatureParameters getSignatureParameters() {
        return switch (getLevel().getSignatureForm()) {
            ...
            default -> throw new UnsupportedSignatureLevelException(getLevel().name());
        };
    }
